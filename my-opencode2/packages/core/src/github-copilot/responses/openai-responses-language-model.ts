...
export class OpenAIResponsesLanguageModel implements LanguageModelV3 {
  ...
  private async getArgs({
    ...
    const openaiOptions = await parseResponsesProviderOptions(this.config.provider, providerOptions)
    const resolvedReasoning = resolveResponsesReasoning(openaiOptions)
    const treatAsReasoningModel = modelConfig.isReasoningModel || Object.keys(resolvedReasoning).length > 0
    ...
    const baseArgs = {
      ...
      ...(Object.keys(resolvedReasoning).length > 0 && {
...
function normalizeReasoningToken(value: unknown): string | undefined {
  if (typeof value !== "string") return undefined
  const token = value.trim()
  if (!token || /^\d+$/.test(token) || !/^[A-Za-z][A-Za-z0-9_-]*$/.test(token)) return undefined
  return token
}
...
export function resolveResponsesReasoning(
  ...
  if (!options) return {}
  const effort = normalizeReasoningToken(
    options.reasoningEffort ?? options.reasoning_effort ?? options.reasoning?.effort,
  )
  const summary = normalizeReasoningToken(options.reasoningSummary ?? options.reasoning?.summary)
  return {
    ...(effort ? { effort } : {}),
    ...(summary ? { summary } : {}),
  }
...
async function parseResponsesProviderOptions(
  provider: string,
  providerOptions: LanguageModelV3CallOptions["providerOptions"],
) {
  const keys = ["copilot", "openai", provider.split(".")[0]?.trim()].filter((key, index, all): key is string => {
    return Boolean(key) && all.indexOf(key) === index
  })
  const merged: OpenAIResponsesProviderOptions = {}

...
it.instance(
  "custom Claude provider uses Anthropic SDK instead of OpenAI-compatible SDK",
  Effect.gen(function* () {
    const provider = yield* Provider.Service
    const providers = yield* provider.list()
    const custom = providers[ProviderID.make("claude-test")]
    expect(custom).toBeDefined()

    const model = yield* provider.getModel(ProviderID.make("claude-test"), ModelID.make("claude-sonnet-test"))
    expect(model.api.npm).toBe("@ai-sdk/anthropic")
    expect(model.api.url).toBe("https://claude.aiapis.help/v1")

    const language = yield* provider.getLanguage(model)
    expect(language.provider).toBe("claude-test")
    expect(language.provider).not.toBe("claude-test.chat")
    expect(language.modelId).toBe("claude-sonnet-test")
  }),
  {
    config: {
      provider: {
        "claude-test": {
          name: "Claude Test",
          npm: "@ai-sdk/anthropic",
          api: "https://claude.aiapis.help/v1",
          env: [],
          options: {
            apiKey: "test-claude-key",
            baseURL: "https://claude.aiapis.help/v1",
          },
          models: {
            "claude-sonnet-test": {
...
it.instance(
  "custom gpt and claude prefix models infer reasoning and image capabilities when config only declares names",
  Effect.gen(function* () {
    const provider = yield* Provider.Service
    const gpt = yield* provider.getModel(ProviderID.make("custom-openai-compatible"), ModelID.make("gpt-4o-test"))
    expect(gpt.capabilities.reasoning).toBe(true)
    expect(gpt.capabilities.attachment).toBe(true)
    expect(gpt.capabilities.input.image).toBe(true)
    expect(gpt.capabilities.output.text).toBe(true)
    expect(gpt.api.npm).toBe("@opencode-ai/openai-responses")
    expect(gpt.capabilities.interleaved).toBe(false)
    expect(gpt.limit).toMatchObject({ context: 128_000, output: 16_384 })
    expect(Object.keys(gpt.variants ?? {})).toContain("high")

    const nestedGpt = yield* provider.getModel(
      ProviderID.make("custom-openai-compatible"),
      ModelID.make("openai/gpt-4.1"),
    )
    expect(nestedGpt.capabilities.reasoning).toBe(true)
    expect(nestedGpt.capabilities.attachment).toBe(true)
    expect(nestedGpt.capabilities.input.image).toBe(true)
    expect(nestedGpt.api.npm).toBe("@opencode-ai/openai-responses")
    expect(nestedGpt.capabilities.interleaved).toBe(false)
    expect(nestedGpt.limit).toMatchObject({ context: 1_047_576, output: 32_768 })

    const gpt55 = yield* provider.getModel(ProviderID.make("custom-openai-compatible"), ModelID.make("gpt-5.5"))
    expect(gpt55.capabilities.reasoning).toBe(true)
    expect(gpt55.capabilities.input.image).toBe(true)
    expect(gpt55.api.npm).toBe("@opencode-ai/openai-responses")
    expect(gpt55.limit).toMatchObject({ context: 1_050_000, output: 128_000 })
    expect(gpt55.limit.input).toBeUndefined()

    const claude = yield* provider.getModel(ProviderID.make("claude-test"), ModelID.make("claude-3-5-sonnet-test"))
    expect(claude.api.npm).toBe("@ai-sdk/anthropic")
    expect(claude.capabilities.reasoning).toBe(true)
    expect(claude.capabilities.attachment).toBe(true)
    expect(claude.capabilities.input.image).toBe(true)
    expect(claude.capabilities.output.text).toBe(true)
    expect(claude.capabilities.interleaved).toBe(false)
    expect(claude.limit).toMatchObject({ context: 200_000, output: 64_000 })
    expect(Object.keys(claude.variants ?? {})).toContain("high")

    const claudeSonnet46 = yield* provider.getModel(ProviderID.make("claude-test"), ModelID.make("claude-sonnet-4-6"))
    expect(claudeSonnet46.capabilities.reasoning).toBe(true)
    expect(claudeSonnet46.capabilities.input.image).toBe(true)
    expect(claudeSonnet46.limit).toMatchObject({ context: 1_000_000, output: 64_000 })

    const grok = yield* provider.getModel(ProviderID.make("custom-openai-compatible"), ModelID.make("grok-4.6"))
    ...
    const kimi = yield* provider.getModel(ProviderID.make("custom-openai-compatible"), ModelID.make("kimi-k3"))
...
it.instance(
  "config accepts attempt-indexed timeout lists",
  Effect.gen(function* () {
    yield* setProcessEnv("ANTHROPIC_API_KEY", "env-api-key")
    const providers = yield* Provider.Service.use((provider) => provider.list())
    expect(providers[ProviderID.anthropic]).toBeDefined()
    expect(providers[ProviderID.anthropic].options.timeout).toEqual([180_000, 300_000, 480_000])
    expect(providers[ProviderID.anthropic].options.chunkTimeout).toEqual([30_000, 45_000, 60_000])
    expect(providers[ProviderID.anthropic].options.firstEventTimeout).toEqual([30_000, 60_000, 120_000])
  }),
  {
    config: {
      provider: {
        anthropic: {
          options: {
            timeout: [180_000, 300_000, 480_000],
            chunkTimeout: [30_000, 45_000, 60_000],
            firstEventTimeout: [30_000, 60_000, 120_000],
          },
        },
      },
    },
...
test("custom provider model inherits built-in catalog limit when output is omitted", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await withTestInstance({
    ...
    fn: async (ctx) => {
      const providers = await list(ctx)
      const model = providers[ProviderID.make("custom-openai")].models["gpt-5"]
...
test("custom provider apiMode controls OpenAI-compatible API package", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
        JSON.stringify({
          ...
          provider: {
            "chat-provider": {
              ...
              models: {
                "gpt-chat": { name: "GPT Chat" },
              },
            },
            "responses-provider": {
              name: "Responses Provider",
              npm: "@ai-sdk/openai-compatible",
              api: "https://responses.example.com/v1",
              options: {
              ...
              models: {
                "gpt-responses": { name: "GPT Responses" },
              },
            },
            "default-provider": {
              name: "Default Provider",
              npm: "@ai-sdk/openai-compatible",
              api: "https://default.example.com/v1",
              options: {
                apiKey: "default-key",
                baseURL: "https://default.example.com/v1",
              },
              models: {
                "gpt-default": { name: "GPT Default" },
              },
            },
          },
        }),
  ...
  await withTestInstance({
    directory: tmp.path,
    fn: async (ctx) => {
      const chat = await getModel(ProviderID.make("chat-provider"), ModelID.make("gpt-chat"), ctx)
      const responses = await getModel(ProviderID.make("responses-provider"), ModelID.make("gpt-responses"), ctx)
      const fallback = await getModel(ProviderID.make("default-provider"), ModelID.make("gpt-default"), ctx)

      expect(chat.api.npm).toBe("@ai-sdk/openai-compatible")
      expect(responses.api.npm).toBe("@opencode-ai/openai-responses")
      expect(fallback.api.npm).toBe("@opencode-ai/openai-responses")
      expect(chat.api.url).toBe("https://chat.example.com/v1")
      expect(responses.api.url).toBe("https://responses.example.com/v1")
      expect(fallback.api.url).toBe("https://default.example.com/v1")
    },
  })
})
...
test("explicit variant config replaces generated variants", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await withTestInstance({
    ...
    fn: async (ctx) => {
      ...
      const model = providers[ProviderID.anthropic].models["claude-sonnet-4-20250514"]
      expect(model.variants!["high"]).toBeDefined()
      expect(model.variants!["low"]).toBeUndefined()
...
test("variant config can replace generated variants", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opencode.json"),
  ...
  await withTestInstance({
    ...
    fn: async (ctx) => {
      await set(ctx, "ANTHROPIC_API_KEY", "test-api-key")
      const model = (await list(ctx))[ProviderID.anthropic].models["claude-sonnet-4-20250514"]
      expect(Object.keys(model.variants ?? {})).toEqual(["focused"])
      expect(model.variants?.focused.thinking.budgetTokens).toBe(12000)
    },
  })
})

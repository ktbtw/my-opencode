...
describe("ProviderTransform.options - setCacheKey", () => {
  ...
  test("should set promptCacheKey for custom gpt models by default", () => {
    const customGptModel = {
      ...mockModel,
      id: "custom/gpt-5.5",
    ...
    const result = ProviderTransform.options({ model: customGptModel, sessionID, providerOptions: {} })
    expect(result.promptCacheKey).toBe(sessionID)
  })

  test("should route custom openai-package gpt promptCacheKey under openai provider options", () => {
    const customGptModel = {
      ...mockModel,
      id: "custom/gpt-5.5",
      providerID: "custom-gpt",
    ...
    const options = ProviderTransform.options({ model: customGptModel, sessionID, providerOptions: {} })
    expect(ProviderTransform.providerOptions(customGptModel, options)).toEqual({
      openai: expect.objectContaining({ promptCacheKey: sessionID }),
    })
  })

  test("should set promptCacheKey for openai-compatible gpt models by default", () => {
    const compatibleGptModel = {
      ...mockModel,
      id: "custom/gpt-5.5",
      providerID: "custom-compatible-gpt",
      api: {
        id: "gpt-5.5",
        url: "https://example.com/v1",
    ...
    const result = ProviderTransform.options({ model: compatibleGptModel, sessionID, providerOptions: {} })
    expect(result.promptCacheKey).toBe(sessionID)
  })

  test("should not set promptCacheKey for gpt models when providerOptions.setCacheKey is false", () => {
    const customGptModel = {
      ...mockModel,
      id: "custom/gpt-5.5",
      providerID: "custom-gpt",
  ...
  test("should not set promptCacheKey for openai provider when providerOptions.setCacheKey is false", () => {
    const openaiModel = {
      ...mockModel,
      providerID: "openai",
      api: {
  ...
  test("should not set promptCacheKey for azure gpt models when providerOptions.setCacheKey is false", () => {
    const azureModel = {
      ...mockModel,
      providerID: "azure",
      api: {
  ...
  test("should set openrouter prompt_cache_key by default", () => {
    ...
    const result = ProviderTransform.options({ model: openrouterModel, sessionID, providerOptions: {} })
    expect(result.promptCacheKey).toBeUndefined()
    expect(result.prompt_cache_key).toBe(sessionID)
  })

  test("should not set openrouter prompt_cache_key when providerOptions.setCacheKey is false", () => {
    const openrouterModel = {
      ...mockModel,
      providerID: "openrouter",
      api: {
  ...
  test("should set promptCacheKey for azure gpt models by default", () => {
    const azureModel = {
      ...mockModel,
      providerID: "azure",
      api: {
...
describe("ProviderTransform.options - Grok reasoning", () => {
  const sessionID = "test-session-grok"

  const createModel = (npm: string, providerID: string, reasoning = true) =>
    ({
      id: `${providerID}/grok-4`,
      providerID,
      api: {
        id: "grok-4",
  ...
  test("enables reasoning by default for native xAI", () => {
    const result = ProviderTransform.options({
      model: createModel("@ai-sdk/xai", "xai"),
      sessionID,
      providerOptions: {},
    })
  ...
  test("enables reasoning by default for OpenRouter Grok", () => {
    const result = ProviderTransform.options({
      model: createModel("@openrouter/ai-sdk-provider", "openrouter"),
      sessionID,
      providerOptions: {},
    })
  ...
  test("exposes variants when the model metadata omits reasoning capability", () => {
    const model = createModel("@ai-sdk/xai", "xai", false)
...
describe("ProviderTransform.variants", () => {
  ...
  describe("@openrouter/ai-sdk-provider", () => {
    test("returns conservative variants for capability-declared models", () => {
      const model = createMockModel({
    ...
    test("all Grok models return low and high with reasoning", () => {
      const model = createMockModel({
  ...
  describe("@ai-sdk/xai", () => {
    test("all Grok models return low and high with reasoningEffort", () => {
      const model = createMockModel({
        id: "xai/grok-3",
        providerID: "xai",
        api: {
  ...
  describe("@ai-sdk/openai-compatible", () => {
    ...
    test("returns GPT-5 effort variants with xhigh for custom gpt-5.5 providers", () => {
      const model = createMockModel({
        id: "custom-provider/gpt-5.5",
        providerID: "custom-provider",
        api: {
          id: "gpt-5.5",
          url: "https://api.custom.com",

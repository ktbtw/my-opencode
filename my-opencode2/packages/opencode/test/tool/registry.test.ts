...
describe("tool.registry", () => {
  ...
  it.instance("exposes goal completion only in goal mode", () =>
    Effect.gen(function* () {
      const registry = yield* ToolRegistry.Service
      ...
      expect((yield* registry.tools(input)).some((tool) => tool.id === "goal_complete")).toBe(false)
      expect((yield* registry.tools(input)).some((tool) => tool.id === "question")).toBe(true)
      expect((yield* registry.tools({ ...input, goalMode: true })).some((tool) => tool.id === "question")).toBe(false)
      expect((yield* registry.tools({ ...input, goalMode: true })).some((tool) => tool.id === "goal_complete")).toBe(
        true,
      )
    }),
  )

...
describe("tool.shell background jobs", () => {
  it.live("returns quick auto commands inline", () =>
    runIn(
      projectRoot,
      Effect.gen(function* () {
        const jobs = yield* BackgroundJob.Service
        const result = yield* run({
          command: execute('console.log("quick")'),
          description: "Run quick command",
        })
  ...
  it.live("auto-yields long commands and preserves their result", () =>
    runIn(
      projectRoot,
      Effect.gen(function* () {
        const jobs = yield* BackgroundJob.Service
        const result = yield* run({
          command: execute('console.log("started"); await Bun.sleep(200); console.log("finished")'),
          description: "Run managed command",
          mode: "auto",
          yield_time_ms: 20,
        })
        ...
        const done = yield* jobs.wait({ id: String(jobID), timeout: 2_000 })
        expect(done.info?.status).toBe("completed")
        expect(done.info?.output).toContain("finished")
      }),
    ),
  )

  it.live("keeps released auto jobs alive after the tool turn aborts", () =>
    runIn(
      projectRoot,
      Effect.gen(function* () {
        const jobs = yield* BackgroundJob.Service
        const seen = yield* Deferred.make<SessionPrompt.PromptInput>()
        const turn = new AbortController()
        const promptOps: PromptOps = {
          cancel: () => Effect.void,
          resolvePromptParts: (template) => Effect.succeed([{ type: "text", text: template }]),
          prompt: (input) => Deferred.succeed(seen, input).pipe(Effect.andThen(Effect.never)),
          loop: () => Effect.never,
        }
        const result = yield* run(
          {
            command: execute('await Bun.sleep(200); console.log("survived turn cleanup")'),
            description: "Run beyond tool turn",
            mode: "auto",
            yield_time_ms: 20,
          },
          { ...ctx, abort: turn.signal, extra: { promptOps } },
        )
        const jobID = String(result.metadata.jobId)

        expect(result.metadata.state).toBe("running")
        turn.abort()
        const done = yield* jobs.wait({ id: jobID, timeout: 2_000 })
        expect(done.info?.status).toBe("completed")
        expect(done.info?.output).toContain("survived turn cleanup")
        expect(done.info?.output).not.toContain("User aborted the command")

        const injected = yield* awaitWithTimeout(
          Deferred.await(seen),
          "background completion was not injected after turn cleanup",
          "2 seconds",
        )
        const text = injected.parts.find((part) => part.type === "text")?.text ?? ""
        expect(text).toContain(jobID)
        expect(text).toContain("survived turn cleanup")
      }),
    ),
  )

  it.live("starts and stops persistent background commands", () =>
    runIn(
      projectRoot,
      Effect.gen(function* () {
        const jobs = yield* BackgroundJob.Service
        const result = yield* run({
          command: execute('console.log("ready"); await Bun.sleep(30000)'),
          description: "Run persistent command",
          mode: "background",
        })
        const jobID = String(result.metadata.jobId)
        const live = yield* pollWithTimeout(
          jobs.get(jobID).pipe(Effect.map((info) => (info?.output?.includes("ready") ? info : undefined))),
          "background command did not publish live output",
        )

        expect(live.status).toBe("running")
        const stopped = yield* jobs.cancel(jobID)
        expect(stopped?.status).toBe("cancelled")
      }),
    ),
  )

  it.live("injects completion into the owning conversation", () =>
    runIn(
      projectRoot,
      Effect.gen(function* () {
        const seen = yield* Deferred.make<SessionPrompt.PromptInput>()
        ...
        const result = yield* run(
          {
            command: execute('console.log("completed output")'),
            description: "Run completion command",
            mode: "background",
          },
          { ...ctx, extra: { promptOps } },
        )
        const injected = yield* awaitWithTimeout(
          Deferred.await(seen),
          "background completion was not injected",
          "2 seconds",
        )
        const text = injected.parts.find((part) => part.type === "text")?.text ?? ""

        expect(result.metadata.state).toBe("running")
        expect(injected.noReply).toBe(true)
        expect(text).toContain(String(result.metadata.jobId))

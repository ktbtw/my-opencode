...
describe("tool.question", () => {
  it.instance("should successfully execute with valid question parameters", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* tool
        .execute(
          {
            reason: "material_preference_conflict",
            blocking_context:
              "The user must choose a color theme because the variants have materially different branding.",
            attempted_steps: ["Checked the existing theme tokens", "Compared both available variants"],
            questions,
          },
          ctx,
        )
        ...
        .execute(
          {
            ...
            blocking_context: "The target file was not provided and no project file matches the requested name.",
            attempted_steps: ["Searched the project tree", "Checked recent conversation attachments"],
            questions,
          },
          ctx,
        )
  ...
  it.instance("rejects a question without a blocking reason or attempted steps", () =>
    Effect.gen(function* () {
      const toolInfo = yield* QuestionTool
      const tool = yield* toolInfo.init()
      ...
      expect(result.title).toBe("Question rejected")
      expect(result.output).toContain("已尝试步骤")
    }),
  )
})

...
describe("tool parameters", () => {
  ...
  describe("shell", () => {
    ...
    test("accepts execution mode + auto yield time", () => {
      ...
      expect(parsed.yield_time_ms).toBe(250)
    })
  ...
  describe("question", () => {
    test("accepts questions array", () => {
      const parsed = parse(Question, {
        ...
        blocking_context: "A required project-specific value is missing.",
        attempted_steps: ["Searched the project files"],
        questions: [
          {
            question: "pick one",
            header: "Header",
    ...
    test("rejects missing gate fields", () => {
      expect(
        accepts(Question, {
          questions: [],
        }),

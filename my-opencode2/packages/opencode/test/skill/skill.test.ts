...
const itWithSemanticAgentIsolation = testEffect(
  Layer.mergeAll(
    Skill.layer.pipe(
      Layer.provide(Discovery.defaultLayer),
      Layer.provide(Config.defaultLayer),
      ...
      Layer.provide(AppFileSystem.defaultLayer),
      Layer.provide(Global.layer),
      Layer.provide(RuntimeFlags.layer({ semanticAgentIsolation: true })),
    ),
    node,
  ),
)
...
`,
  ...
  itWithSemanticAgentIsolation.live("only loads explicitly configured skill paths when isolated", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
...
`,
              ...
              Bun.write(
                path.join(dir, "semantic-skills", "reverse-expert", "SKILL.md"),
                `---
name: reverse-expert
description: Launcher-managed reverse engineering skill.
---
...
`,
          ...
          expect(list.map((s) => s.name)).toEqual(["reverse-expert"])
          expect(list[0].location).toContain(path.join("semantic-skills", "reverse-expert", "SKILL.md"))
          expect(yield* skill.get("customize-opencode")).toBeUndefined()
        }),
      { git: true, config: { skills: { paths: ["semantic-skills"], urls: [] } } },
    ),
  )

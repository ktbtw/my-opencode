...
describe("InstanceStore", () => {
  ...
  it.live("keeps a shared instance alive until all leases are released", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
  ...
  it.live("memory worker release does not interrupt an active task lease", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
      ...
      const taskContext = yield* store.load({ directory: dir })
      const task = yield* Effect.gen(function* () {
        yield* Deferred.succeed(taskStarted, undefined)
        yield* Deferred.await(releaseTask)
        expect(taskContext.directory).toBe(dir)
      }).pipe(Effect.forkScoped)

      yield* Deferred.await(taskStarted)
      const memoryContext = yield* store.load({ directory: dir })
      yield* store.dispose(memoryContext)

      expect(disposed).toEqual([])
      yield* Deferred.succeed(releaseTask, undefined)
      yield* Fiber.join(task)

      yield* store.dispose(taskContext)
      expect(disposed).toEqual([dir])
    }),
  )

  it.live("does not dispose the previous generation during reload", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
      const disposed: string[] = []

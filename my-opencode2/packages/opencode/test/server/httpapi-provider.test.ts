...
function providerIDs(input: unknown, key: "all" | "providers") {
  return providerList(input, key)
    .map((provider) => (isRecord(provider) && typeof provider.id === "string" ? provider.id : undefined))
    .filter((id): id is string => !!id)
}

function modelIDs(input: unknown, key: "all" | "providers", id: string) {
  const provider = providerByID(input, key, id)
...
describe("provider HttpApi", () => {
  it.instance(
    "serves legacy provider list shape for launcher and frontend",
    Effect.gen(function* () {
      const instance = yield* TestInstance
      yield* setEnvScoped("ANTHROPIC_API_KEY", "test-api-key")
      ...
      expect(Array.isArray(isRecord(body) ? body.all : undefined)).toBe(true)
      expect(isRecord(body) && isRecord(body.default)).toBe(true)
      expect(Array.isArray(isRecord(body) ? body.connected : undefined)).toBe(true)
      expect(providerIDs(body, "all")).toContain("anthropic")
      expect(isRecord(body) && isRecord(body.default) && typeof body.default.anthropic).toBe("string")
      expect(isRecord(body) && Array.isArray(body.connected) && body.connected).toContain("anthropic")
      expect(modelIDs(body, "all", "anthropic").length).toBeGreaterThan(0)
    }),
    projectOptions,
  )

  it.instance(
    "applies provider enable and disable filters to legacy provider list",
    Effect.gen(function* () {
      const instance = yield* TestInstance
      yield* setEnvScoped("ANTHROPIC_API_KEY", "test-api-key")
      yield* setEnvScoped("OPENAI_API_KEY", "test-openai-key")
      ...
      const body = yield* Effect.promise(() => response.json())
      const ids = providerIDs(body, "all")
      expect(ids).toContain("anthropic")
      expect(ids).not.toContain("openai")
      expect(isRecord(body) && Array.isArray(body.connected) && body.connected).toContain("anthropic")
      expect(isRecord(body) && Array.isArray(body.connected) && body.connected).not.toContain("openai")
    }),
    {
      config: {
        formatter: false,
  ...
  it.instance(
    "serves v2 provider and model lists alongside the legacy provider contract",
    Effect.gen(function* () {
      const instance = yield* TestInstance
      yield* setEnvScoped("ANTHROPIC_API_KEY", "test-api-key")
      ...
      const legacyResponse = yield* Effect.promise(() => Promise.resolve(server.request("/provider", { headers })))
      const v2ProviderResponse = yield* Effect.promise(() =>
        Promise.resolve(server.request("/api/provider", { headers })),
      )
      const v2ModelResponse = yield* Effect.promise(() => Promise.resolve(server.request("/api/model", { headers })))

      expect(legacyResponse.status).toBe(200)
      expect(v2ProviderResponse.status).toBe(200)
      expect(v2ModelResponse.status).toBe(200)

      const legacyBody = yield* Effect.promise(() => legacyResponse.json())
      const v2Providers = yield* Effect.promise(() => v2ProviderResponse.json())
      const v2Models = yield* Effect.promise(() => v2ModelResponse.json())

      expect(Array.isArray(isRecord(legacyBody) ? legacyBody.all : undefined)).toBe(true)
      expect(isRecord(legacyBody) && isRecord(legacyBody.default)).toBe(true)
      expect(Array.isArray(isRecord(legacyBody) ? legacyBody.connected : undefined)).toBe(true)
      expect(Array.isArray(v2Providers)).toBe(true)
      expect(Array.isArray(v2Models)).toBe(true)
      expect(providerIDs(legacyBody, "all")).toContain("anthropic")
      expect(
        (v2Providers as unknown[]).some((provider) => isRecord(provider) && provider.id === "anthropic"),
      ).toBe(true)
      expect(
        (v2Models as unknown[]).some(
          (model) => isRecord(model) && model.providerID === "anthropic" && typeof model.id === "string",
        ),
      ).toBe(true)
    }),
    projectOptions,

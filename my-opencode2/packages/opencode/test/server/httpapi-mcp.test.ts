...
const mcpToolsFixture = fileURLToPath(new URL("../fixture/mcp-tools-server.ts", import.meta.url))
      ...
      Effect.gen(function* () {
        ...
        const added = yield* request(handler, McpPaths.status, tmp.directory, {
          ...
          body: JSON.stringify({
            ...
            config: {
              type: "local",
              command: ["bun", mcpToolsFixture],
              timeout: 5_000,
            },
          }),
        })
        expect(added.status).toBe(200)
        expect(yield* json(added)).toMatchObject({ fixture: { status: "connected" } })

        const response = yield* request(handler, McpPaths.tools, tmp.directory)

        expect(response.status).toBe(200)
        expect(yield* json(response)).toMatchObject({
          fixture: [
            {
              id: "fixture_search_docs",
              name: "search_docs",

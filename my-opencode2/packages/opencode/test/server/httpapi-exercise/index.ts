...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .at((ctx) => ({
	  ...
	  http.protected
	    .get("/session/{sessionID}/todo/history", "session.todoHistory")
	    .seeded((ctx) =>
	      Effect.gen(function* () {
	        const session = yield* ctx.session({ title: "Todo history session" })
	        const completed = yield* ctx.todos(session.id, [
	          { content: "cover completed todo history", status: "completed", priority: "high" },
	        ])
	        yield* Effect.promise(() => new Promise((resolve) => setTimeout(resolve, 2)))
	        const active = yield* ctx.todos(session.id, [
	          { content: "cover active todo history", status: "pending", priority: "medium" },
	        ])
	        return { session, completed, active }
	      }),
	    )
	    .at((ctx) => ({
	      path: route("/session/{sessionID}/todo/history", { sessionID: ctx.state.session.id }),
	      headers: ctx.headers(),
	    }))
	    .json(200, (body, ctx) => {
	      array(body)
	      check(body.length === 2, "todo history should keep both seeded batches")
	      check(stable(body[0]) === stable(ctx.state.active), "latest active batch should be first")
	      check(stable(body[1]) === stable(ctx.state.completed), "completed batch should remain in history")
	    }),
  http.protected

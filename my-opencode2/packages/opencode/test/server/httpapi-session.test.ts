...
const it = testEffect(
  Layer.mergeAll(instanceStoreLayer, Project.defaultLayer, Session.defaultLayer, Todo.defaultLayer, workspaceLayer),
)
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const todoSession = yield* createSession({ title: "todo history" })
        ...
        const completed = yield* todo.update({
          ...
          todos: [{ content: "完成接口历史第一批", status: "completed", priority: "high" }],
        })
        yield* Effect.promise(() => new Promise((resolve) => setTimeout(resolve, 2)))
        const active = yield* todo.update({
          sessionID: todoSession.id,
          todos: [{ content: "接口历史第二批进行中", status: "pending", priority: "medium" }],
        })
        ...
        expect(
          yield* requestJson<unknown[]>(pathFor(SessionPaths.todoHistory, { sessionID: parent.id }), { headers }),
        ).toEqual([])

        const currentTodo = yield* requestJson<Array<{ planID?: string; content?: string }>>(
          pathFor(SessionPaths.todo, { sessionID: todoSession.id }),
          { headers },
        )
        expect(currentTodo).toHaveLength(1)
        expect(currentTodo[0]?.planID).toBe(active[0]?.planID)

        const todoHistory = yield* requestJson<Array<Array<{ planID?: string; content?: string }>>>(
          pathFor(SessionPaths.todoHistory, { sessionID: todoSession.id }),
          { headers },
        )
        expect(todoHistory).toHaveLength(2)
        expect(todoHistory[0]?.[0]?.planID).toBe(active[0]?.planID)
        expect(todoHistory[1]?.[0]?.planID).toBe(completed[0]?.planID)
  ...
  it.instance(
    "accepts image, pdf, and text file attachments through prompt endpoint",
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const headers = { "x-opencode-directory": test.directory, "content-type": "application/json" }
        const session = yield* createSession({ title: "attachments" })
        const photon = yield* Effect.promise(() => import("@silvia-odwyer/photon-node"))
        const source = new photon.PhotonImage(
          new Uint8Array(Array.from({ length: 4 * 4 * 4 }, (_, index) => (index % 4 === 3 ? 255 : index % 251))),
          4,
          4,
        )
        const image = Buffer.from(source.get_bytes()).toString("base64")
        source.free()
        const pdf = Buffer.from("%PDF-1.4\n").toString("base64")
        const note = Buffer.from("plain attachment body").toString("base64")

        const created = yield* requestJson<MessageV2.WithParts>(
            ...
            body: JSON.stringify({
              ...
              parts: [
                ...
                },
                {
                  ...
                  mime: "text/plain",
                  filename: "note.txt",
                  url: `data:text/plain;base64,${note}`,
                },
              ],
            }),
          },
        ...
        expect(created.parts).toEqual(
          expect.arrayContaining([
            expect.objectContaining({ type: "text", text: "please inspect attachments" }),
            expect.objectContaining({ type: "file", mime: "image/png", filename: "screenshot.png" }),
            expect.objectContaining({ type: "file", mime: "application/pdf", filename: "brief.pdf" }),
            expect.objectContaining({ type: "file", mime: "text/plain", filename: "note.txt" }),
            expect.objectContaining({ type: "text", synthetic: true, text: "plain attachment body" }),
          ]),
        )

        const model = {
          id: ModelID.make("test-attachments"),
        ...
        const modelMessages = yield* Effect.promise(() => MessageV2.toModelMessages([created], model))
        expect(modelMessages).toEqual([
          {
            ...
            content: [
              ...
              { type: "text", text: "plain attachment body" },
            ],
          },
        ])
      }),
    {
      ...
      config: {
        ...
        model: "test-provider/test-attachments",
        provider: {
          "test-provider": {
            name: "Test Provider",
            env: [],

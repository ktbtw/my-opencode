...
const original = {
  ...
  relayOperatorKey: process.env.OPENCODE_RELAY_OPERATOR_KEY,
}
...
afterEach(async () => {
  ...
  if (original.relayOperatorKey === undefined) delete process.env.OPENCODE_RELAY_OPERATOR_KEY
  else process.env.OPENCODE_RELAY_OPERATOR_KEY = original.relayOperatorKey
...
async function startListener() {
  ...
  delete process.env.OPENCODE_RELAY_OPERATOR_KEY
  return Server.listen({ hostname: "127.0.0.1", port: await availablePort() })
}
...
describe("HttpApi Server.listen", () => {
  test("serves launcher-compatible health JSON instead of UI fallback", async () => {
    const listener = await startNoAuthListener()
    try {
      const health = await fetch(new URL("/health", listener.url))
      expect(health.status).toBe(200)
      expect(health.headers.get("content-type")).toContain("application/json")
      expect(await health.json()).toMatchObject({ healthy: true, ok: true, version: expect.any(String) })

      const globalHealth = await fetch(new URL("/global/health", listener.url))
      expect(globalHealth.status).toBe(200)
      expect(await globalHealth.json()).toMatchObject({ healthy: true, ok: true, version: expect.any(String) })
    } finally {
      await stop(listener, "timed out cleaning up health listener").catch(() => undefined)
    }
  })

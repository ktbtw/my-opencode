...
describe("session.llm-native.request", () => {
    ...
    const messages: ModelMessage[] = [
      ...
      },
      {
        ...
        content: [
          ...
          { type: "file", mediaType: "application/pdf", filename: "doc.pdf", data: new URL("data:application/pdf;base64,JVBERg==") },
          { type: "image", mediaType: "image/jpeg", image: new URL("https://cdn.example.test/photo.jpg") },
        ],
      },
      {
        role: "assistant",
    ...
    expect(request.messages).toMatchObject([
      {
        ...
        content: [
          ...
          { type: "media", mediaType: "application/pdf", filename: "doc.pdf", data: "data:application/pdf;base64,JVBERg==" },
          { type: "media", mediaType: "image/jpeg", data: "https://cdn.example.test/photo.jpg" },
        ],
      },
      {
        role: "assistant",
  ...
  test("selects native routes from existing provider packages", () => {
    ...
    expect(
      LLMNative.model({
        ...
        api: { ...baseModel.api, id: "gpt-4o", npm: "@ai-sdk/openai-compatible" },
      }),
    ).toMatchObject({
      route: "openai-compatible-chat",
      baseURL: "https://api.openai.com/v1",
  ...
  test("enables native runtime for Anthropic API-key models", () => {
    const model = {
      ...
      api: { ...baseModel.api, npm: "@ai-sdk/anthropic", url: "https://claude.aiapis.help/v1" },
    }
    const provider = {
      ...providerInfo,
      ...
      options: { apiKey: "test-anthropic-key", baseURL: "https://claude.aiapis.help/v1" },
    }
    expect(
      LLMNativeRuntime.status({
        model,
    ...
    const result = LLMNativeRuntime.stream({
      ...
      auth: undefined,
      llmClient: {
        stream: ({ request }: { request: ReturnType<typeof LLMNative.request> }) => {
          expect(request.model.route).toBe("anthropic-messages")
          expect(request.model.route).not.toBe("openai-compatible-chat")
          expect(String(request.model.provider)).toBe("anthropic")
          expect(request.model.baseURL).toBe("https://claude.aiapis.help/v1")
          expect(request.model.apiKey).toBe("test-anthropic-key")
  ...
  test("enables native runtime for custom Claude providers that use Anthropic SDK", () => {
    const model = {
      ...baseModel,
      id: ModelID.make("claude-sonnet-test"),
      providerID: ProviderID.make("claude-test"),
      api: {
        id: "claude-sonnet-test",
        npm: "@ai-sdk/anthropic",
        url: "https://claude.aiapis.help/v1",
      },
    }
    const provider = {
      ...providerInfo,
      id: ProviderID.make("claude-test"),
      name: "Claude Test",
      env: [],
      options: { apiKey: "test-claude-key", baseURL: "https://claude.aiapis.help/v1" },
    }
    ...
    const result = LLMNativeRuntime.stream({
      ...
      llmClient: {
        stream: ({ request }: { request: ReturnType<typeof LLMNative.request> }) => {
          expect(request.model.route).toBe("anthropic-messages")
          expect(request.model.route).not.toBe("openai-compatible-chat")
          expect(String(request.model.provider)).toBe("claude-test")
          expect(String(request.model.id)).toBe("claude-sonnet-test")
          expect(request.model.baseURL).toBe("https://claude.aiapis.help/v1")
          expect(request.model.apiKey).toBe("test-claude-key")
          return Effect.succeed([])
        },
      } as any,
      isOpenaiOauth: false,
  ...
  test("routes custom OpenAI Responses models through native runtime with OpenAI options", () => {
    const model = {
      ...baseModel,
      id: ModelID.make("gpt-5.4"),
      providerID: ProviderID.make("超级便宜"),
      api: {
        id: "gpt-5.4",
        npm: "@ai-sdk/openai",
        url: "https://ikik.net/v1",
      },
    }
    const provider = {
      ...providerInfo,
      id: ProviderID.make("超级便宜"),
      name: "超级便宜",
      env: [],
      options: { apiKey: "test-cheap-key", baseURL: "https://ikik.net/v1" },
    }

    expect(LLMNativeRuntime.status({ model, provider, auth: undefined })).toMatchObject({
      type: "supported",
      apiKey: "test-cheap-key",
      baseURL: "https://ikik.net/v1",
    })
    ...
    const result = LLMNativeRuntime.stream({
      ...
      llmClient: {
        stream: ({ request }: { request: ReturnType<typeof LLMNative.request> }) => {
          expect(request.model.route).toBe("openai-responses")
          expect(String(request.model.provider)).toBe("超级便宜")
          expect(String(request.model.id)).toBe("gpt-5.4")
          expect(request.model.baseURL).toBe("https://ikik.net/v1")
          expect(request.model.apiKey).toBe("test-cheap-key")
  ...
  test("compiles native OpenAI Responses multimodal messages", async () => {
    const prepared = await Effect.runPromise(
      LLMClient.prepare(
        LLMNative.request({
          model: baseModel,
          messages: [
            {
              ...
              content: [
                ...
                { type: "file", mediaType: "image/png", filename: "img.png", data: "iVBORw0KGgo=" },
                { type: "file", mediaType: "application/pdf", filename: "doc.pdf", data: new URL("data:application/pdf;base64,JVBERg==") },
              ],
            },
          ],
        ...
        input: [
          {
            ...
            content: [
              ...
              { type: "input_file", filename: "doc.pdf", file_data: "data:application/pdf;base64,JVBERg==" },
            ],
          },
        ],
      },

...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("filters legacy empty output retry synthetic text", async () => {
    const messageID = "m-user"
        ...
        parts: [
          {
            ...basePart(messageID, "p1"),
            type: "text",
            text: "系统检测到你上一轮模型响应正常结束，但没有返回任何可展示的正文。",
            synthetic: true,
            metadata: { empty_output_retry: true },
          },
        ] as MessageV2.Part[],
      },
    ]
  ...
  test("filters empty assistant turns that only contain structural parts", async () => {
    const userID = "m-user"
...
describe("session.message-v2.fromError", () => {
  ...
  test("serializes embedded provider rate limit errors from response validation failures", () => {
    const body = {
      error: {
    ...
    const error = new Error(
      `Type validation failed: Value: ${JSON.stringify(body)}.\nError message: [{"code":"invalid_union"}]`,
    )
  ...
  test("serializes OpenAI response stream read errors as retryable APIError", () => {
    const body = {
      type: "error",
      sequence_number: 0,
      error: {
  ...
  test("serializes timeout errors as retryable APIError", () => {
    const error = new DOMException("LLM first stream event timed out after 12000ms", "TimeoutError")
    const result = MessageV2.fromError(error, { providerID })

...
describe("session.compaction.isOverflow", () => {
  ...
  it.live(
    "includes reasoning tokens when total usage is unavailable",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 100_000, output: 32_000 })
...
describe("session.compaction.process", () => {
  ...
  itCompaction.instance(
    "uses a bounded local summary when the reduced payload still overflows",
    Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const session = yield* ssn.create({})
      const msg = yield* createUserMessage(session.id, "hello")
      ...
      if (summary?.info.role === "assistant") {
        expect(summary.info.finish).toBe("end_turn")
        expect(summary.info.error).toBeUndefined()
        expect(summary.parts.some((part) => part.type === "text" && part.text.includes("## Critical Context"))).toBe(
          true,
        )
      }
  ...
  itCompaction.instance(
    "keeps the previous summary and recent conversation in local overflow recovery",
    () =>
      Effect.gen(function* () {
        const ssn = yield* SessionNs.Service
        const test = yield* TestInstance
        const session = yield* ssn.create({})
        yield* createUserMessage(session.id, "old context")
        yield* createCompactionMarker(session.id)
        const firstMarker = (yield* ssn.messages({ sessionID: session.id })).at(-1)?.info.id
        ...
        yield* createUserMessage(session.id, "recent progress after the anchor")
        yield* createCompactionMarker(session.id)
        ...
        const recovered = summaries.at(-1)?.parts.find((part) => part.type === "text")

        expect(result).toBe("continue")
        expect(recovered?.type === "text" ? recovered.text : "").toContain("anchored previous summary")
        expect(recovered?.type === "text" ? recovered.text : "").toContain("recent progress after the anchor")
      }).pipe(withCompaction({ result: "compact" })),
    { git: true },
  )

  itCompaction.instance("retries provider context overflow with a reduced payload", () => {
    const stub = llm()
    let recovered = ""
    stub.push(
      Stream.fail(
        new APICallError({
          ...
          statusCode: 400,
          responseHeaders: {},
          responseBody: '{"type":"error","error":{"code":"context_length_exceeded"}}',
          isRetryable: false,
        }),
      ),
    )
    stub.push(reply("recovered summary", (input) => (recovered = JSON.stringify(input.messages))))

    return Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const bus = yield* Bus.Service
      const session = yield* ssn.create({})
      const msg = yield* createUserMessage(session.id, "recover this conversation")
      const msgs = yield* ssn.messages({ sessionID: session.id })
      let errors = 0
      const unsub = yield* bus.subscribeCallback(SessionNs.Event.Error, (event) => {
        if (event.properties.sessionID === session.id) errors++
      })
      yield* Effect.addFinalizer(() => Effect.sync(unsub))
      ...
      expect(result).toBe("continue")
      expect(recovered).toContain("conversation-history")
      expect(recovered).toContain("recover this conversation")
      expect(errors).toBe(0)
      expect(summaries).toHaveLength(1)
      expect(summaries[0]?.info.role === "assistant" ? summaries[0].info.error : undefined).toBeUndefined()
  ...
  itCompaction.instance("preflights a huge single turn into the fallback input budget", () => {
    const stub = llm()
    let estimated = Number.POSITIVE_INFINITY
    stub.push(reply("bounded summary", (input) => (estimated = Token.estimate(JSON.stringify(input.messages)))))
    const provider = ProviderTest.fake({ model: createModel({ context: 4_000, output: 1_000 }) })

    return Effect.gen(function* () {
      const ssn = yield* SessionNs.Service
      const session = yield* ssn.create({})
      const msg = yield* createUserMessage(session.id, `begin-${"x".repeat(100_000)}-end`)
      const msgs = yield* ssn.messages({ sessionID: session.id })
      ...
      expect(result).toBe("continue")
      expect(estimated).toBeLessThanOrEqual(1_500)
      const summaries = (yield* ssn.messages({ sessionID: session.id })).filter(
        (item) => item.info.role === "assistant" && item.info.summary,
      )
      expect(summaries).toHaveLength(1)
  ...
  itCompaction.instance(
    "honors an explicit two-turn tail while summarizing only the head",
    () => {
      const stub = llm()
      let captured = ""
      stub.push(
  ...
  itCompaction.instance(
    "keeps six recent turns verbatim by default",
    () => {
      const stub = llm()
      let captured = ""
      stub.push(
      ...
      return Effect.gen(function* () {
        ...
        for (let index = 1; index <= 8; index++) {
          yield* createUserMessage(session.id, `turn-${index}`)
        }
        yield* createCompactionMarker(session.id)
        ...
        expect(captured).toContain("turn-2")
        for (let index = 3; index <= 8; index++) {
          expect(captured).not.toContain(`turn-${index}`)
        }
      }).pipe(withCompaction({ llm: stub.layer }))
    },
    { git: true },
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(captured).toContain("append-only fact ledger")
        expect(captured).toContain("Never replace a concrete error")
        expect(captured).toContain("mark the old fact as stale")
      }).pipe(withCompaction({ llm: stub.layer }))
    },
    { git: true },
  )

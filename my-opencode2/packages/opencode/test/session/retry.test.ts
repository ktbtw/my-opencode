...
describe("session.retry.retryable", () => {
  ...
  test("caps retry policy at five total attempts by default", async () => {
    const error = apiError({ "retry-after-ms": "0" })
    let updates = 0
      ...
      Schedule.toStepWithMetadata(
        SessionRetry.policy({
          ...
          set: (info) =>
            Effect.sync(() => {
              updates += 1
              expect(info.maxAttempts).toBe(SessionRetry.RETRY_MAX_ATTEMPTS)
            }),
        }),
      ),
    )

    for (let i = 0; i < 6; i++) await Effect.runPromise(step(error).pipe(Effect.ignore))

    expect(updates).toBe(SessionRetry.RETRY_MAX_ATTEMPTS - 1)
  })
...
describe("session.message-v2.fromError", () => {
  ...
  test("retries embedded provider concurrency limit errors", () => {
    const body = {
      error: {
    ...
    const result = MessageV2.fromError(
      new Error(`Type validation failed: Value: ${JSON.stringify(body)}.\nError message: []`),
      { providerID: ProviderID.make("openai") },
    )
  ...
  test("retries OpenAI response stream read errors", () => {
    const body = {
      type: "error",
      sequence_number: 0,
      error: {

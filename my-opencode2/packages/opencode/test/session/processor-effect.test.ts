  ...
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        expect(handle.compactionReason).toBe("threshold")
  ...
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        const msg = yield* assistant(chat.id, parent.id, path.resolve(dir))
        const mdl = yield* provider.getModel(ref.providerID, ref.modelID)
        const deltas: Array<{ delta: string; partType?: string }> = []
        const seen = defer<void>()
        const off = yield* bus.subscribeCallback(MessageV2.Event.PartDelta, (evt) => {
          if (evt.properties.sessionID !== chat.id) return
          deltas.push({ delta: evt.properties.delta, partType: evt.properties.partType })
          if (evt.properties.partType === "reasoning") seen.resolve()
        })
        const handle = yield* processors.create({
          assistantMessage: msg,
          sessionID: chat.id,
        ...
        off()
        expect(deltas).toContainEqual({ delta: "think", partType: "reasoning" })
        expect(deltas).toContainEqual({ delta: "done", partType: undefined })
      }),
    { config: (url) => providerCfg(url) },
  ),
)
  ...
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        expect(value).toBe("compact")
        expect(handle.compactionReason).toBe("provider_context_overflow")
        expect(yield* llm.calls).toBe(1)
        expect(handle.message.error).toBeUndefined()
      }),
    { config: (url) => providerCfg(url) },

...
const png = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO7Z0XQAAAAASUVORK5CYII="

const testPng = Effect.fn("test.png")(function* () {
  const photon = yield* Effect.promise(() => import("@silvia-odwyer/photon-node"))
  const source = new photon.PhotonImage(
    new Uint8Array(Array.from({ length: 8 * 8 * 4 }, (_, index) => (index % 4 === 3 ? 255 : index % 251))),
    8,
    8,
  )
  const bytes = source.get_bytes()
  source.free()
...
const legacyImageConfig = (url: string) =>
  ({
...
const latestCompactionPart = Effect.fn("test.latestCompactionPart")(function* (sessionID: SessionID) {
  const session = yield* Session.Service
...
noLLMServer.instance(
  "image model stores multiple generated images and strips prompt args",
  () =>
    Effect.gen(function* () {
      ...
      const server = Bun.serve({
        ...
        async fetch(req) {
          ...
          return new Response(
            JSON.stringify({
              data: [
                { b64_json: png, revised_prompt: "clean skyline" },
                { b64_json: png },
              ],
            }),
      ...
      yield* writeConfig(directory, imageConfig(`${server.url.origin}/v1`))

      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({ title: "Image regression" })
      const result = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        parts: [{ type: "text", text: "draw a skyline --n=2 --quality=high" }],
      })

      expect(hit).toBe("/v1/images/generations")
      expect(body?.prompt).toBe("draw a skyline")
      ...
      const files = result.parts.filter((part): part is MessageV2.FilePart => part.type === "file")
      const text = result.parts.filter((part): part is MessageV2.TextPart => part.type === "text")
      expect(files).toHaveLength(2)
      expect(files[0]?.filename).toBe("test-image-1.png")
      expect(files[1]?.filename).toBe("test-image-2.png")
      expect(files.every((file) => file.url.startsWith("data:image/png;base64,"))).toBe(true)
      expect(text.some((part) => part.synthetic === true && part.text === "clean skyline")).toBe(true)
    }),
  { git: true },
)

noLLMServer.instance(
  "image model infers legacy gpt-image config and uses generation endpoint",
  () =>
    Effect.gen(function* () {
      let body: Record<string, unknown> | undefined
      let hit = ""
      ...
      const { directory } = yield* TestInstance
      yield* writeConfig(directory, legacyImageConfig(`${server.url.origin}/v1`))

      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({ title: "Legacy image config" })
      const result = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        parts: [{ type: "text", text: "生成一个好看的图片" }],
      })

      expect(hit).toBe("/v1/images/generations")
      expect(body?.model).toBe("gpt-image-2")
      expect(body?.prompt).toBe("生成一个好看的图片")
      expect(result.parts.some((part) => part.type === "file" && part.filename === "gpt-image-2-1.png")).toBe(true)
    }),
  { git: true },
)

noLLMServer.instance(
  "image model sends attached reference images through edits endpoint",
  () =>
    Effect.gen(function* () {
      ...
      const image = yield* testPng()
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({ title: "Image edit regression" })
      const result = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        parts: [
          { type: "file", mime: "image/png", filename: "ref.png", url: `data:image/png;base64,${image}` },
          { type: "text", text: "make it blue --n 2 --fidelity high" },
        ],
      })

      expect(hit).toBe("/v1/images/edits")
      expect(contentType).toContain("multipart/form-data")
      expect(body?.get("prompt")).toBe("make it blue")
      expect(body?.get("n")).toBe("2")
      expect(body?.get("input_fidelity")).toBe("high")
      const img = body?.get("image")
      expect(img).toBeInstanceOf(File)
      expect((img as File | null)?.name).toBe("ref.png")
      expect((img as File | null)?.type).toBe("image/png")
...
noLLMServer.instance(
  "image model records retry metadata and Chinese error after repeated upstream failures",
  () =>
    Effect.gen(function* () {
      let calls = 0
      const server = Bun.serve({
        ...
        async fetch(req) {
          ...
          calls++
          return new Response(JSON.stringify({ error: { message: "internal image failure" } }), {
            status: 500,
            headers: { "Content-Type": "application/json" },
          })
        },
      ...
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({ title: "Image retry regression" })
      const result = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        parts: [{ type: "text", text: "draw a retry case" }],
      })
      ...
      if (!MessageV2.APIError.isInstance(result.info.error)) throw new Error("expected image APIError")
      expect(result.info.error.data.message).toContain("已自动重试3次仍失败")
      expect(result.info.error.data.metadata).toMatchObject({
        source: "image",
        kind: "server",
        attempts: "3",
        max_attempts: "3",
...
it.instance("provider context overflow marks compaction as overflow", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const session = yield* sessions.create({ title: "Provider overflow" })
    yield* prompt.prompt({
      sessionID: session.id,
      agent: "build",
      noReply: true,
      parts: [{ type: "text", text: "large request" }],
    })
    ...
    const part = yield* latestCompactionPart(session.id)
    expect(part?.auto).toBe(true)
    expect(part?.overflow).toBe(true)
  }),
)

it.instance("token threshold compaction is not marked as provider overflow", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    const session = yield* sessions.create({ title: "Threshold overflow" })
    yield* prompt.prompt({
      sessionID: session.id,
      agent: "build",
      noReply: true,
      parts: [{ type: "text", text: "many tokens" }],
    })
    yield* llm.text("after", { usage: { input: 1_000_000, output: 1 } })
    yield* llm.text("summary")
    yield* prompt.loop({ sessionID: session.id })

    const part = yield* latestCompactionPart(session.id)
    expect(part?.auto).toBe(true)
    expect(part?.overflow).not.toBe(true)
  }),
)

...
interface MockClientState {
  ...
  lastCallToolOptions?: Record<string, unknown>
...
function getOrCreateClientState(name?: string): MockClientState {
  ...
  if (!state) {
    state = {
      ...
      callToolCalls: 0,
      callToolDelay: 0,
      callToolResult: { content: [{ type: "text", text: "tool done" }] },
...
void mock.module("@modelcontextprotocol/sdk/client/index.js", () => ({
  Client: class MockClient {
    ...
    async callTool(_input: unknown, _schema: unknown, options?: Record<string, unknown>) {
      if (this._state) {
        this._state.callToolCalls++
        this._state.lastCallToolOptions = options
      }
      const delay = this._state?.callToolDelay ?? 0
...
it.instance(
  "uses a phase-specific tool timeout instead of the startup timeout",
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        const serverState = getOrCreateClientState("default")

        const tools = yield* mcp.tools()
        const tool = tools["timeout-server_test_tool"]
        expect(tool?.execute).toBeDefined()
        yield* Effect.promise(() => (tool!.execute as any)({}, {}))
        expect(serverState.callToolCalls).toBe(1)
        expect(serverState.lastCallToolOptions?.timeout).toBe(3_600_000)
      }),
    ),
  {
    config: {
...
it.instance(
  "runs configured MCP tools as asynchronous jobs and returns the final result",
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        const serverState = getOrCreateClientState("default")
        serverState.callToolDelay = 25
        serverState.callToolResult = {
          content: [{ type: "text", text: "analysis ready" }],
          structuredContent: { session_id: "ida-session-1" },
        }
        const tools = yield* mcp.tools()
        const started = (yield* Effect.promise(() =>
          (tools["async-server_test_tool"].execute as any)({}, {}),
        )) as any
        expect(started.structuredContent.mcp_job.status).toBe("running")
        expect(started.structuredContent.mcp_job.timeout_ms).toBe(3_600_000)

        const jobID = started.structuredContent.mcp_job.id
        const completed = (yield* Effect.promise(() =>
          (tools["async-server_job"].execute as any)({ job_id: jobID, wait_ms: 1_000 }, {}),
        )) as any
        expect(completed.structuredContent.mcp_job.status).toBe("completed")
        expect(completed.structuredContent.session_id).toBe("ida-session-1")
        expect(completed.content.some((item: any) => item.text === "analysis ready")).toBe(true)
      }),
    ),
  {
    config: {
...
it.instance(
  "toolStatus() exposes cached tool metadata grouped by server",
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        lastCreatedClientName = "tool-status-server"
        const serverState = getOrCreateClientState("tool-status-server")
        serverState.tools = [
          {
            name: "search_docs",
        ...
        yield* mcp.add("tool-status-server", {
          type: "local",
          command: ["echo", "test"],
        })

        const status = yield* mcp.toolStatus()
        expect(status["tool-status-server"]).toEqual([
          {
            id: "tool-status-server_search_docs",
            name: "search_docs",
            description: "Search docs",
            inputSchema: {
              type: "object",
  ...
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        ...
        if (status["bad-server"]?.status === "failed") {
          expect(status["bad-server"].error).toContain("listTools failed")
        }

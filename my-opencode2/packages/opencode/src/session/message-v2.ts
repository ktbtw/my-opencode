...
function isEmptyOutputRetryPart(part: Part) {
  return part.type === "text" && part.synthetic === true && part.metadata?.empty_output_retry === true
}

function isRenderableAssistantPart(part: Part) {
...
function isEmptyAssistantMessage(msg: WithParts) {
  return msg.info.role === "assistant" && !msg.parts.some(isRenderableAssistantPart)
}
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "user") {
      ...
      for (const part of msg.parts) {
        if (isEmptyOutputRetryPart(part)) continue

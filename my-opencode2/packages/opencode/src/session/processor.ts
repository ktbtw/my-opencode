...
import { count as tokenCount, isOverflow } from "./overflow"
...
import { normalizeStreamDeltaText } from "@/util/stream-delta"
import { GlobalBus } from "@/bus/global"

const DOOM_LOOP_THRESHOLD = 3
const log = Log.create({ service: "session.processor" })

function usageTotal(tokens: MessageV2.Assistant["tokens"]) {
  return tokens.total || tokens.input + tokens.output + tokens.reasoning + tokens.cache.read + tokens.cache.write
}

export type CompactionReason = "threshold" | "provider_context_overflow"
...
interface ProcessorContext extends Input {
  ...
  blocked: boolean
  compaction: { reason: CompactionReason } | undefined
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "reasoning-delta":
            ...
            const reasoningDelta = normalizeStreamDeltaText(value.text)
          ...
          case "step-finish": {
            ...
            const contextTokens = tokenCount(usage.tokens)
            const contextLimit = ctx.model.limit.context
            const contextUsagePercent =
              contextTokens !== undefined && contextTokens > 0 && Number.isFinite(contextLimit) && contextLimit > 0
                ? Math.max(0, Math.round((contextTokens / contextLimit) * 1000) / 10)
                : undefined
            GlobalBus.emit("event", {
              payload: {
                ...
                parent_message_id: ctx.assistantMessage.parentID,
                ...
                compaction_count_tokens: contextTokens,
                input_tokens: usage.tokens.input,
                output_tokens: usage.tokens.output,
                reasoning_tokens: usage.tokens.reasoning,
                cache_read_tokens: usage.tokens.cache.read,
                cache_write_tokens: usage.tokens.cache.write,
                total_tokens: usageTotal(usage.tokens),
                cost: usage.cost,
              },
            })
            if (!ctx.assistantMessage.summary) {
            ...
            if (
              ...
              ctx.compaction = { reason: "threshold" }
            }
            return
          }
          ...
          case "text-delta":
            ...
            const textDelta = normalizeStreamDeltaText(value.text)
            if (!textDelta) return
      ...
      const halt = Effect.fn("SessionProcessor.halt")(function* (e: unknown) {
        ...
        if (MessageV2.ContextOverflowError.isInstance(error)) {
          ctx.compaction = { reason: "provider_context_overflow" }
          if (!ctx.assistantMessage.summary) {
      ...
      const process = Effect.fn("SessionProcessor.process")(function* (streamInput: LLM.StreamInput) {
        ...
        return yield* Effect.gen(function* () {
          yield* Effect.gen(function* () {
            ctx.currentText = undefined
            ctx.reasoningMap = {}
            const retryStatus = yield* status.get(ctx.sessionID)
            const streamAttempt = retryStatus.type === "retry" ? retryStatus.attempt + 1 : 1
            yield* status.set(ctx.sessionID, { type: "busy" })
            const stream = llm.stream({ ...streamInput, attempt: streamAttempt })

            yield* stream.pipe(
              Stream.tap((event) => handleEvent(event)),
              Stream.takeUntil(() => ctx.compaction !== undefined),
              Stream.runDrain,
            )
          }).pipe(
            Effect.onInterrupt(() =>
          ...
          if (ctx.compaction) return "compact"

...
function providerOptions(input: Pick<StreamInput, "model" | "providerOptions">) {
  const options = ProviderTransform.providerOptions(input.model, input.providerOptions ?? {})
  if (input.model.api.npm === "@ai-sdk/openai") return { openai: options.openai ?? input.providerOptions ?? {} }
  return options
}
...
export function stream(input: StreamInput): StreamResult {
  ...
  return {
    ...
    stream: input.llmClient.stream({
      ...
      activeTools: Object.keys(input.tools).filter((name) => name !== "invalid"),
    }),
  }
}

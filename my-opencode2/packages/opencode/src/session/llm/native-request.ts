...
const fileData = (part: Record<string, unknown>) => {
  if (typeof part.data === "string" || part.data instanceof Uint8Array) return part.data
  if (part.data instanceof ArrayBuffer) return new Uint8Array(part.data)
  if (part.data instanceof URL) return part.data.toString()
  if (typeof part.url === "string") return part.url
  if (part.url instanceof URL) return part.url.toString()
  return undefined
}

const imageData = (part: Record<string, unknown>) => {
  if (typeof part.image === "string" || part.image instanceof Uint8Array) return part.image
  if (part.image instanceof ArrayBuffer) return new Uint8Array(part.image)
  if (part.image instanceof URL) return part.image.toString()
  return undefined
}

const mediaPart = (part: Record<string, unknown>) => {
  const data = fileData(part)
  if (data === undefined)
    throw new Error("Native LLM request adapter only supports file parts with data, URL, or Uint8Array data")
  return {
    type: "media" as const,
...
const imagePart = (part: Record<string, unknown>) => {
  const data = imageData(part)
  if (data === undefined)
    throw new Error("Native LLM request adapter only supports image parts with image URL, data URL, or Uint8Array data")
  return {
    type: "media" as const,
...
const route = (model: Provider.Model) => ROUTE[model.api.npm]
...
export const model = (input: Provider.Model | RequestInput, headers?: Record<string, string>) => {
  ...
  const routeID = route(model)
  if (!routeID) throw new Error(`Native LLM request adapter does not support provider package ${model.api.npm}`)
  return LLM.model({
    id: model.api.id,

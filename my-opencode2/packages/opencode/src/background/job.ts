  ...
  Effect.gen(function* () {
      ...
      const result = yield* SynchronizedRef.modify(
        ...
        (jobs): readonly [FinishResult, Map<string, Active>] => {
          ...
          const next = {
            ...
            info: {
              ...job.info,
              status: job.cancel_requested && status === "completed" ? "cancelled" : status,
              completed_at,
    ...
    const update: Interface["update"] = Effect.fn("BackgroundJob.update")(function* (input) {
      return yield* SynchronizedRef.modify(
        (yield* InstanceState.get(state)).jobs,
        (jobs): readonly [Info | undefined, Map<string, Active>] => {
    ...
    const wait: Interface["wait"] = Effect.fn("BackgroundJob.wait")(function* (input) {
      ...
      const current = (yield* SynchronizedRef.get((yield* InstanceState.get(state)).jobs)).get(input.id)
      return { info: current ? snapshot(current) : undefined, timedOut: true }
    })
    ...
    const cancel: Interface["cancel"] = Effect.fn("BackgroundJob.cancel")(function* (id) {
      const job = yield* SynchronizedRef.modify(
        ...
        (jobs): readonly [Active | undefined, Map<string, Active>] => {
          ...
          if (!current || current.info.status !== "running") return [current, jobs]
          const next = { ...current, cancel_requested: true }
          return [next, new Map(jobs).set(id, next)]
        },
      )
      if (!job) return
      if (job.info.status !== "running") return snapshot(job)
      if (job.cancel) {
        yield* job.cancel.pipe(Effect.ignore)
        const done = yield* Deferred.await(job.done).pipe(Effect.timeoutOption(5_000))
        if (done._tag === "Some") return done.value
      }
      if (job.fiber) {
    ...
    const remove: Interface["remove"] = Effect.fn("BackgroundJob.remove")(function* (id) {
      return yield* SynchronizedRef.modify(
        (yield* InstanceState.get(state)).jobs,
        (jobs): readonly [boolean, Map<string, Active>] => {
          const job = jobs.get(id)
    ...
    return Service.of({ list, get, start, update, wait, cancel, remove })
  }),
)

...
const DEFAULT_REQUEST_TIMEOUT = 900_000
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  [ProviderTransform.OPENAI_COMPATIBLE_RESPONSES_NPM]: () =>
...
const imageOutputModelPattern = /(^|[-_/])(gpt-image|dall-e|imagen|flux|recraft|sdxl|stable-diffusion)([-_/]|$)/i
const modelAliasPrefixPattern = /^(?:gpt|claude)/
const visionModelAliasPrefixPattern = /^(?:gpt|claude|grok|gemini|kimi)/
...
function inferGptLimit(alias: string): InferredCapabilities["limit"] | undefined {
  if (/^gpt[-_.]?4o(?:$|[-_.])/.test(alias) || /^chatgpt[-_.]?4o(?:$|[-_.])/.test(alias)) {
    return { context: 128_000, output: 16_384 }
  }
  if (/^gpt[-_.]?4\.?1(?:$|[-_.])/.test(alias)) {
    return { context: 1_047_576, output: 32_768 }
  }
  if (/^gpt[-_.]?5\.(?:4|5)(?:$|[-_.])/.test(alias)) {
    return { context: 1_050_000, output: 128_000 }
  }
  if (/^gpt[-_.]?5(?:$|[-_.])/.test(alias)) {
    return { context: 400_000, output: 128_000 }
  }
  return undefined
}

function inferClaudeLimit(alias: string): InferredCapabilities["limit"] | undefined {
  if (/^claude[-_.]?opus[-_.]?4[-_.]?7(?:$|[-_.])/.test(alias)) {
    return { context: 1_000_000, output: 128_000 }
  }
  if (/^claude[-_.]?sonnet[-_.]?4[-_.]?6(?:$|[-_.])/.test(alias)) {
    return { context: 1_000_000, output: 64_000 }
  }
  if (/^claude[-_.]?haiku[-_.]?4[-_.]?5(?:$|[-_.])/.test(alias)) {
    return { context: 200_000, output: 64_000 }
  }
  if (/^claude(?:$|[-_.])/.test(alias)) {
    return { context: 200_000, output: 64_000 }
  }
  return undefined
}
...
export function inferConfigModelCapabilities(
  ...
  const aliases = [
    modelID,
    modelName,
    lowerModelID.includes("/") ? lowerModelID.split("/").at(-1) : undefined,
    modelName.includes("/") ? modelName.toLowerCase().split("/").at(-1) : undefined,
  ]
  ...
  if (isImageOutputModel) {
    inferred.input = { text: true, audio: false, image: false, video: false, pdf: false }
    inferred.output = { text: false, audio: false, image: true, video: false, pdf: false }
    inferred.attachment = false
    inferred.toolcall = false
  }
  if (!isImageOutputModel && isGptOrClaudeFamily) {
    ...
    if (apiNpm === "@ai-sdk/openai-compatible") inferred.interleaved = { field: "reasoning_content" }
    inferred.limit =
  ...
  if (apiNpm === "@ai-sdk/openai-compatible" && lowerModelID.includes("deepseek")) {
    inferred.reasoning = true
    inferred.interleaved = { field: "reasoning_content" }
  }
...
function findCatalogModel(input: {
  ...
  const byProvider = input.database[input.providerID]
  const direct =
    byProvider?.models[input.modelID] ??
    byProvider?.models[input.apiID] ??
    Object.values(byProvider?.models ?? {}).find(
      (model) => model.api.id === input.apiID || model.name === input.name || model.id === input.modelID,
    )
  if (direct) return direct

  for (const provider of Object.values(input.database)) {
    const matched =
      provider.models[input.modelID] ??
      provider.models[input.apiID] ??
      Object.values(provider.models).find(
        (model) => model.api.id === input.apiID || model.id === input.modelID || model.name === input.name,
      )
...
type ProviderAPIMode = "chat" | "responses"

const GPT_PROVIDER_API_MODE_RE = /(?:^|\/)(?:chatgpt|gpt)(?:[._-]|\d|$)/

function parseProviderAPIMode(value: unknown): ProviderAPIMode | undefined {
  if (typeof value !== "string") return undefined
  if (value.trim().toLowerCase() === "responses") return "responses"
  if (value.trim().toLowerCase() === "chat") return "chat"
  return undefined
}

function defaultProviderAPIMode(npm: string, ...modelIDs: string[]): ProviderAPIMode {
  if (npm === "@ai-sdk/openai-compatible") {
    const isGPT = modelIDs.some((id) => GPT_PROVIDER_API_MODE_RE.test(id.trim().toLowerCase()))
    return isGPT ? "responses" : "chat"
  }
  return "responses"
}

function normalizeProviderAPIMode(value: unknown, fallback: ProviderAPIMode = "responses"): ProviderAPIMode {
  return parseProviderAPIMode(value) ?? fallback
}

function providerNpmForAPIMode(npm: string, apiMode: ProviderAPIMode) {
  if (apiMode === "responses" && npm === "@ai-sdk/openai-compatible") {
    return ProviderTransform.OPENAI_COMPATIBLE_RESPONSES_NPM
  }
  return npm
}

function selectSDKLanguageModel(
  sdk: any,
  modelID: string,
  options: Record<string, any> | undefined,
  fallback: ProviderAPIMode,
) {
  const apiMode = normalizeProviderAPIMode(options?.apiMode, fallback)
  if (apiMode === "chat" && sdk.chat) return sdk.chat(modelID)
  if (apiMode === "responses" && sdk.responses) return sdk.responses(modelID)
  if (sdk.messages && apiMode === "responses") return sdk.messages(modelID)
  return sdk.languageModel(modelID)
}

function selectAzureLanguageModel(sdk: any, modelID: string, useChat: boolean) {
  return selectSDKLanguageModel(sdk, modelID, { apiMode: useChat ? "chat" : "responses" }, "responses")
}
    ...
    openai: () =>
      Effect.succeed({
        ...
        async getModel(sdk: any, modelID: string, _options?: Record<string, any>) {
          return selectSDKLanguageModel(sdk, modelID, _options, "responses")
        },
        options: {},
      }),
    xai: () =>
    ...
    "github-copilot": () =>
      Effect.succeed({
        ...
        async getModel(sdk: any, modelID: string, _options?: Record<string, any>) {
          ...
          return selectSDKLanguageModel(
            sdk,
            modelID,
            _options,
            shouldUseCopilotResponsesApi(modelID) ? "responses" : "chat",
          )
        },
        options: {},
      }),
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          ...
          const configuredAPIMode = parseProviderAPIMode(provider.options?.apiMode)
          const parsed: Info = {
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            ...
            const modelProviderNpm = model.provider?.npm
            const apiNpmSource =
              modelProviderNpm ??
              provider.npm ??
            ...
            const providerAPIMode =
              configuredAPIMode ??
              (modelProviderNpm === "@ai-sdk/openai-compatible"
                ? "chat"
                : defaultProviderAPIMode(apiNpmSource, apiID, modelID, model.name ?? ""))
            const apiNpm = providerNpmForAPIMode(apiNpmSource, providerAPIMode)
            const name = iife(() => {
            ...
            const parsedModel: Model = {
              ...
              capabilities: {
                ...
                reasoning: model.reasoning ?? inferred?.reasoning ?? existingModel?.capabilities.reasoning ?? false,
                attachment: model.attachment ?? inferred?.attachment ?? existingModel?.capabilities.attachment ?? false,
                toolcall: model.tool_call ?? inferred?.toolcall ?? existingModel?.capabilities.toolcall ?? true,
                input: {
                  text:
            ...
            // Explicit model variants are the complete contract for this model.
            // Mixing them with inferred variants can send unsupported reasoning
            // parameters to a provider, so only infer when no config was given.
            const resolved =
              model.variants !== undefined ? model.variants : ProviderTransform.variants(parsedModel)
            parsedModel.variants = mapValues(
              pickBy(resolved, (v) => !v.disabled),
              (v) => omit(v, ["disabled"]),
            )
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        const chunkTimeout =
          Array.isArray(configuredChunkTimeout) &&
          configuredChunkTimeout.length > 0 &&
          configuredChunkTimeout.every((item) => typeof item === "number" && item > 0)
            ? configuredChunkTimeout[configuredChunkTimeout.length - 1]
            : (configuredChunkTimeout ?? DEFAULT_CHUNK_TIMEOUT)
        const configuredTimeout = options["timeout"]
        options["timeout"] =
          Array.isArray(configuredTimeout) &&
          configuredTimeout.length > 0 &&
          configuredTimeout.every((item) => typeof item === "number" && item > 0)
            ? configuredTimeout[configuredTimeout.length - 1]
            : (configuredTimeout ?? DEFAULT_REQUEST_TIMEOUT)
        delete options["chunkTimeout"]
        delete options["firstEventTimeout"]
        delete options["toolInputTimeout"]
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          if (
            (ProviderTransform.isOpenAIResponsesNpm(model.api.npm) || model.api.npm === "@ai-sdk/azure") &&
            opts.body &&
            opts.method === "POST"
          ) {
    ...
    const getLanguage = Effect.fn("Provider.getLanguage")(function* (model: Model) {
      ...
      const envs = yield* env.all()
      const provider = s.providers[model.providerID]
      const apiMode = normalizeProviderAPIMode(
        provider?.options?.apiMode,
        defaultProviderAPIMode(model.api.npm, model.api.id, model.id),
      )
      const key = `${model.providerID}/${model.id}/${model.api.npm}/${apiMode}`
      ...
      return yield* EffectPromise.refineRejection(
        async () => {
          ...
          const fallbackAPIMode = defaultProviderAPIMode(model.api.npm, model.api.id, model.id)
          const language = s.modelLoaders[model.providerID]
            ? await s.modelLoaders[model.providerID](sdk, model.api.id, modelOptions)
            : selectSDKLanguageModel(sdk, model.api.id, modelOptions, fallbackAPIMode)

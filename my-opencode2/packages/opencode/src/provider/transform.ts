...
export const OPENAI_COMPATIBLE_RESPONSES_NPM = "@opencode-ai/openai-responses"

export function isOpenAIResponsesNpm(npm: string) {
  return npm === "@ai-sdk/openai" || npm === OPENAI_COMPATIBLE_RESPONSES_NPM
}
...
function sdkKey(npm: string): string | undefined {
  switch (npm) {
    ...
    case "@ai-sdk/azure":
      return "azure"
    case "@ai-sdk/openai":
    case OPENAI_COMPATIBLE_RESPONSES_NPM:
      return "openai"
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  ...
  // Grok exposes adjustable reasoning through the xAI/OpenRouter adapters,
  // even when an upstream models snapshot omits the reasoning capability flag.
  if (id.includes("grok")) {
...
export function options(input: {
  ...
  const shouldSetPromptCacheKey = (() => {
    if (input.providerOptions?.setCacheKey === false) return false
    if (input.providerOptions?.setCacheKey === true) return true
    if (input.model.providerID === "openrouter") return false
    if (input.model.providerID === "openai") return true
    if (input.model.providerID.startsWith("opencode")) return true
  ...
  if (
    ...
    isOpenAIResponsesNpm(input.model.api.npm) ||
  ...
  // xAI/OpenRouter reasoning models do not expose their reasoning stream until
  // an effort is selected. A saved variant overrides this default later.
  ...
  if (input.model.providerID === "venice" && input.providerOptions?.setCacheKey !== false) {
    result["promptCacheKey"] = input.sessionID
  }

  if (input.model.providerID === "openrouter" && input.providerOptions?.setCacheKey !== false) {
...
export function smallOptions(model: Provider.Model) {
  ...
  if (
    ...
    isOpenAIResponsesNpm(model.api.npm) ||
...
export function providerOptions(model: Provider.Model, options: { [x: string]: any }) {
  const {
    ...
    firstEventTimeout: _firstEventTimeout,
    toolInputTimeout: _toolInputTimeout,
    ...providerOptions
  } = options
  if (model.api.npm === "@ai-sdk/gateway") {
    ...
    const rest = Object.fromEntries(Object.entries(providerOptions).filter(([k]) => k !== "gateway"))

  ...
  compaction: Schema.optional(
    Schema.Struct({
      ...
      preserve_recent_tokens: Schema.optional(NonNegativeInt).annotate({
        description:
          "Maximum number of tokens from recent turns to preserve verbatim after compaction (default: 8000-24000 depending on context size)",
      }),
      reserved: Schema.optional(NonNegativeInt).annotate({
        description: "Token buffer for compaction. Leaves enough window to avoid overflow during compaction.",
      }),
      threshold_percent: Schema.optional(PositiveInt).annotate({
        description: "Context usage percentage that triggers automatic compaction (default: 80)",
      }),
    }),
  ),

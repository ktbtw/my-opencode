    ...
    HttpApiGroup.make("mcp")
      .add(
        ...
        HttpApiEndpoint.get("tools", McpPaths.tools, {
          query: WorkspaceRoutingQuery,
          success: described(Schema.Record(Schema.String, Schema.Array(MCP.ToolStatus)), "MCP tools by server"),
        }).annotateMerge(
          OpenApi.annotations({
            identifier: "mcp.tools",
            summary: "Get MCP tools",
            description: "Get cached tool definitions for connected Model Context Protocol (MCP) servers.",
          }),
        ),

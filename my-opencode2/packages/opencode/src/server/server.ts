...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    const relay = yield* Effect.sync(() => Relay.start())

    return {
      hostname: opts.hostname,
      port: address.port,
      url: listenerUrl,
      stop: yield* makeStop(state, unpublishMdns, relay),
    }
  },
)
...
function makeStop(state: ListenerState, unpublishMdns: Effect.Effect<void>, relay?: { stop: () => Promise<void> }) {
  return Effect.gen(function* () {
    ...
    const stopRelayOnce = yield* Effect.cached(Effect.promise(() => relay?.stop() ?? Promise.resolve()).pipe(Effect.ignore))

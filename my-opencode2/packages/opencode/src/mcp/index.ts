...
import { mergeMCPEnvironment } from "./environment"

const log = Log.create({ service: "mcp" })
const DEFAULT_CONNECT_TIMEOUT = 30_000
const DEFAULT_DISCOVERY_TIMEOUT = 30_000
const DEFAULT_ASYNC_TOOL_TIMEOUT = 60 * 60 * 1_000
const MCP_JOB_RETENTION_MS = 24 * 60 * 60 * 1_000
const STDERR_TAIL_LIMIT = 4_096

type MCPJobStatus = "running" | "completed" | "failed" | "cancelled"
...
type MCPJobRegistry = Map<string, MCPJob>

function appendStderrTail(current: string, next: string) {
  const combined = current + next
  return combined.length <= STDERR_TAIL_LIMIT ? combined : combined.slice(-STDERR_TAIL_LIMIT)
}

function errorWithStderr(error: string, stderr: string | undefined) {
  const tail = stderr?.trim()
  if (!tail || error.includes(tail)) return error
  return `${error}; stderr: ${tail}`
}

function resolveConnectTimeout(mcp: ConfigMCP.Info) {
  return mcp.connect_timeout ?? mcp.timeout ?? DEFAULT_CONNECT_TIMEOUT
}

function resolveDiscoveryTimeout(mcp: ConfigMCP.Info) {
  return mcp.discovery_timeout ?? mcp.timeout ?? DEFAULT_DISCOVERY_TIMEOUT
}

function resolveToolTimeout(mcp: ConfigMCP.Info, fallback?: number) {
  return mcp.tool_timeout ?? mcp.timeout ?? fallback
}
...
export const ToolStatus = Schema.Struct({
  ...
  name: Schema.String,
  description: Schema.optional(Schema.String),
  inputSchema: Schema.Record(Schema.String, Schema.Any),
  parameters: Schema.Record(Schema.String, Schema.Any),
  status: Status,
}).annotate({ identifier: "MCPToolStatus" })
...
function jobSnapshot(job: MCPJob) {
  return {
    id: job.id,
    server: job.clientName,
    tool: job.toolName,
    status: job.status,
    started_at: job.startedAt,
    updated_at: job.updatedAt,
    elapsed_ms: Math.max(0, job.updatedAt - job.startedAt),
    timeout_ms: job.timeout,
    ...(job.error ? { error: job.error } : {}),
  }
}

function jobEnvelope(job: MCPJob, result?: unknown) {
  const snapshot = jobSnapshot(job)
  const value = result && typeof result === "object" ? (result as Record<string, any>) : undefined
  const existingContent = Array.isArray(value?.content) ? value.content : []
  const structured =
    value?.structuredContent && typeof value.structuredContent === "object" ? value.structuredContent : {}
  return {
    ...(value ?? {}),
    content: [
      {
        ...
        text: JSON.stringify({
          ...
          next:
            job.status === "running"
              ? `Call ${sanitize(job.clientName)}_job with this job_id and action=status until it reaches a terminal state.`
              : undefined,
        }),
      },
      ...existingContent,
    ],
    structuredContent: { ...structured, mcp_job: snapshot },
    isError: job.status === "failed" || job.status === "cancelled" || value?.isError === true,
  }
}

function pruneJobs(jobs: MCPJobRegistry) {
  const cutoff = Date.now() - MCP_JOB_RETENTION_MS
  for (const [id, job] of jobs) {
    if (job.status !== "running" && job.updatedAt < cutoff) jobs.delete(id)
  }
}

function startMcpJob(input: {
  jobs: MCPJobRegistry
  clientName: string
  mcpTool: MCPToolDef
  client: MCPClient
  ...
  pruneJobs(input.jobs)
  const now = Date.now()
  const abort = new AbortController()
  const job: MCPJob = {
    id: `mcpjob_${crypto.randomUUID()}`,
    clientName: input.clientName,
    toolName: input.mcpTool.name,
    status: "running",
    startedAt: now,
    updatedAt: now,
    timeout: input.timeout ?? DEFAULT_ASYNC_TOOL_TIMEOUT,
    abort,
    promise: Promise.resolve(),
  }
  job.promise = callMcpTool(input.client, input.mcpTool, input.args, job.timeout, abort.signal)
    .then((result) => {
  ...
  input.jobs.set(job.id, job)
  return jobEnvelope(job)
}

function convertMcpJobTool(clientName: string, jobs: MCPJobRegistry): Tool {
  return dynamicTool({
    description:
      "Inspect, wait for, or cancel an asynchronous MCP job. Use wait_ms to wait for progress without busy polling.",
    inputSchema: jsonSchema({
      type: "object",
      properties: {
        job_id: { type: "string", description: "Job id returned by an asynchronous MCP tool." },
        action: { type: "string", enum: ["status", "cancel"], description: "Defaults to status." },
        wait_ms: {
          type: "integer",
          minimum: 0,
          maximum: 30_000,
          description: "Wait up to this many milliseconds for the job to finish. Defaults to 20000.",
        },
      },
      required: ["job_id"],
    ...
    async execute(args: unknown) {
      ...
      const id = typeof input.job_id === "string" ? input.job_id : ""
      const job = jobs.get(id)
      if (!job || job.clientName !== clientName) throw new Error(`Unknown MCP job: ${id || "<empty>"}`)
      if (input.action === "cancel" && job.status === "running") {
        job.abort.abort()
      ...
      const wait =
        typeof input.wait_ms === "number" && Number.isFinite(input.wait_ms)
          ? Math.max(0, Math.min(30_000, Math.floor(input.wait_ms)))
          : 20_000
      if (input.action !== "cancel" && job.status === "running" && wait > 0) {
        await Promise.race([job.promise, new Promise((resolve) => setTimeout(resolve, wait))])
        job.updatedAt = Date.now()
      }
      return jobEnvelope(job, job.status === "completed" ? job.result : undefined)
    },
  })
}
...
function convertMcpTool(
  ...
  return dynamicTool({
    ...
    execute: async (args: unknown, context) => {
      ...
      return callMcpTool(client, mcpTool, args, options.timeout, context.abortSignal)
    },
  })
}

function defs(key: string, client: MCPClient, timeout?: number) {
  return listTools(key, client, timeout ?? DEFAULT_DISCOVERY_TIMEOUT).pipe(
    Effect.catch((err) => {
...
export interface Interface {
  ...
  readonly toolStatus: () => Effect.Effect<Record<string, ToolStatus[]>>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const jobs: MCPJobRegistry = new Map()
    ...
    const connectRemote = Effect.fn("MCP.connectRemote")(function* (
      ...
      const connectTimeout = resolveConnectTimeout(mcp)
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      stderrTails.delete(key)
      const transport = new StdioClientTransport({
        stderr: "pipe",
        command: cmd,
        args,
        cwd,
        env: mergeMCPEnvironment(
          process.env,
          cmd === "opencode" ? { BUN_BE_BUN: "1", ...mcp.environment } : mcp.environment,
        ),
      })
      transport.stderr?.on("data", (chunk: Buffer) => {
        const text = chunk.toString()
        stderrTails.set(key, appendStderrTail(stderrTails.get(key) ?? "", text))
      ...
      return yield* connectTransport(transport, connectTimeout).pipe(
        ...
        Effect.catch((error): Effect.Effect<{ client: MCPClient | undefined; status: Status }> => {
          const msg = errorWithStderr(error instanceof Error ? error.message : String(error), stderrTails.get(key))
    ...
    const create = Effect.fn("MCP.create")(function* (key: string, mcp: ConfigMCP.Info) {
      ...
      const listed = yield* listTools(key, mcpClient, resolveDiscoveryTimeout(mcp)).pipe(
        Effect.map((tools) => ({ tools, error: undefined as string | undefined })),
        Effect.catch((error) =>
          Effect.succeed({
            tools: undefined,
      ...
      if (!listed.tools) {
        ...
        const error = errorWithStderr(listed.error ?? "Failed to get tools", stderrTails.get(key))
        log.error("failed to get tools from client", { key, error })
        return { status: { status: "failed", error } } satisfies CreateResult
      }
          ...
          ([key, mcp]) =>
            Effect.gen(function* () {
              ...
              if (result.mcpClient) {
                ...
                watch(s, key, result.mcpClient, bridge, resolveDiscoveryTimeout(mcp))
              }
            }),
    ...
    const toolStatus = Effect.fn("MCP.toolStatus")(function* () {
      const s = yield* InstanceState.get(state)
      ...
      for (const [clientName, status] of Object.entries(s.status)) {
        ...
        result[clientName] = (s.defs[clientName] ?? []).map((mcpTool) => {
          const inputSchema = mcpTool.inputSchema as Record<string, unknown>
          return {
            id: sanitize(clientName) + "_" + sanitize(mcpTool.name),
            name: mcpTool.name,
            description: mcpTool.description ?? "",
            inputSchema,
    ...
    const createAndStore = Effect.fn("MCP.createAndStore")(function* (name: string, mcp: ConfigMCP.Info) {
      ...
      return yield* storeClient(s, name, result.mcpClient, result.defs!, resolveDiscoveryTimeout(mcp))
    })
      ...
      yield* Effect.forEach(
        ...
        ([clientName, client]) =>
          Effect.gen(function* () {
            ...
            const timeout = entry ? resolveToolTimeout(entry, defaultTimeout) : defaultTimeout
            const asyncTools = new Set(entry?.async_tools ?? [])
            for (const mcpTool of listed) {
              result[sanitize(clientName) + "_" + sanitize(mcpTool.name)] = convertMcpTool(mcpTool, client, {
                clientName,
                timeout,
                asynchronous: asyncTools.has(mcpTool.name),
                jobs,
              })
            }
            if (asyncTools.size > 0) {
              result[sanitize(clientName) + "_job"] = convertMcpJobTool(clientName, jobs)
            }
          }),
        { concurrency: "unbounded" },
      )
    ...
    const authenticate = Effect.fn("MCP.authenticate")(function* (mcpName: string) {
      ...
      if (!result.authorizationUrl) {
        ...
        const listed = client ? yield* defs(mcpName, client, resolveDiscoveryTimeout(mcpConfig)) : undefined
        if (!client || !listed) {
          yield* Effect.tryPromise(() => client?.close() ?? Promise.resolve()).pipe(Effect.ignore)
        ...
        return yield* storeClient(s, mcpName, client, listed, resolveDiscoveryTimeout(mcpConfig))
      }

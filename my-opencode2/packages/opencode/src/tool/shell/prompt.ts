...
export function parameterSchema(description: string) {
  return Schema.Struct({
    ...
    mode: Schema.optional(Schema.Literals(["auto", "foreground", "background"])).annotate({
      description:
        "Execution mode. auto (default) returns a job_id when the command exceeds yield-time; foreground waits; background returns immediately",
    }),
    yield_time_ms: Schema.optional(PositiveInt).annotate({
      description: "Milliseconds auto mode waits before returning a background job (default: 8000)",
    }),

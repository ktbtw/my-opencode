...
import { GoalCompleteTool } from "./goal_complete"
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const orchestrate = yield* OrchestrateTool
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const tool = yield* Effect.all({
          ...
          task: Tool.init(task),
          orchestrate: Tool.init(orchestrate),
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === GoalCompleteTool.id) return input.goalMode === true
        if (tool.id === QuestionTool.id) return input.goalMode !== true

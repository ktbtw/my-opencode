...
function sessionTitle(session: ReturnType<TuiPluginApi["state"]["session"]["get"]> | undefined) {
  return session?.title?.replace(/\s+/g, " ").trim()
}

function taskDoneMessage(session: ReturnType<TuiPluginApi["state"]["session"]["get"]> | undefined) {
...
const tui: TuiPlugin = async (api) => {
  ...
  api.event.on("session.status", (event) => {
    ...
    notify(api, sessionID, taskDoneMessage(session), session?.parentID ? "subagent_done" : "done")
  })

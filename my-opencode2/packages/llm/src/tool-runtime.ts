...
export const stream = <T extends Tools>(options: StreamOptions<T>): Stream.Stream<LLMEvent, LLMError> => {
  ...
  const active = new Set(options.activeTools ?? Object.keys(tools))
  const runtimeTools = toDefinitions(Object.fromEntries(Object.entries(tools).filter(([name]) => active.has(name))))

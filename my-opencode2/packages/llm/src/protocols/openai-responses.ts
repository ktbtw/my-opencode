...
const dataUrl = (mediaType: string, data: string) =>
  DATA_URL_PATTERN.test(data) ? data : `data:${mediaType};base64,${data}`

const mediaData = (part: MediaPart) => {
  const data = ProviderShared.mediaBytes(part)
  return dataUrl(part.mediaType, data)
}

const lowerUserMedia = (part: MediaPart): Schema.Schema.Type<typeof OpenAIResponsesUserContent> => {
  const data = ProviderShared.mediaBytes(part)
  if (part.mediaType.toLowerCase().startsWith("image/")) {
    return {
      type: "input_image",
      image_url: REMOTE_URL_PATTERN.test(data) ? data : dataUrl(part.mediaType, data),
    }
  }
  if (REMOTE_URL_PATTERN.test(data))
    return {
      type: "input_file",
...
const lowerMessages = Effect.fn("OpenAIResponses.lowerMessages")(function* (request: LLMRequest) {
  ...
  for (const message of request.messages) {
    if (message.role === "user") {
      const content: Array<Schema.Schema.Type<typeof OpenAIResponsesUserContent>> = []
      for (const part of message.content) {
        ...
        if (part.type === "media") {
          content.push(lowerUserMedia(part))
          continue
        }
      }
...
const reasoningMetadata = (id: string, item: OpenAIResponsesStreamItem) =>
  openaiMetadata({ itemId: id, reasoningEncryptedContent: item.encrypted_content ?? null })
...
const onReasoningSummaryTextDelta = (state: ParserState, event: OpenAIResponsesEvent): StepResult => {
  if (!event.delta) return [state, NO_EVENTS]
  const events: LLMEvent[] = []
  return [
    {
...
const onReasoningOutputItemDone = (state: ParserState, event: OpenAIResponsesEvent): StepResult => {
  const item = event.item
  if (item?.type !== "reasoning" || !item.id) return [state, NO_EVENTS]
  const events: LLMEvent[] = []
  return [
    { ...state, lifecycle: Lifecycle.reasoningEnd(state.lifecycle, events, item.id, reasoningMetadata(item.id, item)) },
    events,
  ]
}
...
const onOutputItemDone = Effect.fn("OpenAIResponses.onOutputItemDone")(function* (
  ...
  if (item.type === "reasoning") {
    return onReasoningOutputItemDone(state, event) satisfies StepResult
  }
...
const step = (state: ParserState, event: OpenAIResponsesEvent) => {
  ...
  if (event.type === "response.reasoning_summary_text.delta")
    return Effect.succeed(onReasoningSummaryTextDelta(state, event))

...
describe("OpenAI Chat route", () => {
  ...
  it.effect("prepares assistant reasoning content for continuation", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare(
        LLM.request({
          id: "req_reasoning",
          model,
          messages: [Message.assistant({ type: "reasoning", text: "Let me think." })],
        }),
      )

      expect(prepared.body).toMatchObject({
        messages: [{ role: "assistant", content: null, reasoning_content: "Let me think." }],
      })
    }),
  )
  ...
  it.effect("parses reasoning_content separately from text deltas", () =>
    Effect.gen(function* () {
      const body = sseEvents(
        deltaChunk({ reasoning_content: "Thinking" }),
        deltaChunk({ reasoning_content: " briefly." }),
      ...
      expect(response.events.slice(0, 7)).toMatchObject([
        ...
        { type: "reasoning-delta", id: "reasoning-0", text: " briefly." },
        { type: "text-start", id: "text-0" },
      ...
      expect(response.toolCalls).toEqual([
        {
          ...
          input: {
            tool: "apply_patch",
            error: "Invalid JSON input for openai-chat tool call apply_patch",
          },
          providerExecuted: undefined,
          providerMetadata: undefined,
        },

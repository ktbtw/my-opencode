...
describe("LLMClient tools", () => {
  ...
  it.effect("keeps inactive tools executable without advertising them", () =>
    Effect.gen(function* () {
      const invalid = tool({
        description: "Do not use",
        parameters: Schema.Struct({ tool: Schema.String, error: Schema.String }),
        success: Schema.Struct({ output: Schema.String }),
        execute: ({ tool, error }) => Effect.succeed({ output: `${tool}: ${error}` }),
      })
      ...
      const layer = dynamicResponse((input) =>
        Effect.sync(() => {
          ...
          return input.respond(
            bodies.length === 1
              ? sseEvents(toolCallChunk("call_1", "invalid", '{"tool":"lookup","error":"bad json"}'), finishChunk("tool_calls"))
              : sseEvents(deltaChunk({ role: "assistant", content: "Done." }), finishChunk("stop")),
      ...
      const events = Array.from(
        yield* TestToolRuntime.runTools({
          request: baseRequest,
          tools: { get_weather, invalid },
          activeTools: ["get_weather"],
        }).pipe(Stream.runCollect, Effect.provide(layer)),
      )

      const first = bodies[0]
      if (!first || typeof first !== "object") throw new Error("Expected first request body")
      expect(Reflect.get(first, "tools")).toMatchObject([{ function: { name: "get_weather" } }])
      expect(Reflect.get(first, "tools")).toHaveLength(1)
      expect(events.find(LLMEvent.is.toolResult)).toMatchObject({
        id: "call_1",
        name: "invalid",
        result: { type: "json", value: { output: "lookup: bad json" } },
      })
    }),
  )
